...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS tb_obs_indicator_detail (
  detail_indicator_id INT NOT NULL,
  detail_region_id INT NOT NULL,
  PRIMARY KEY (detail_indicator_id, detail_region_id),
  CONSTRAINT fk_indicator_detail_indicator
    FOREIGN KEY (detail_indicator_id)
    REFERENCES tb_obs_indicator (indicator_id)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_indicator_details_region
    FOREIGN KEY (detail_region_id)
...
CREATE TABLE IF NOT EXISTS tb_obs_indicator_data_cualitative (
  ...
  indicator_data_total DECIMAL(10,2) NULL,
  indicator_id INT NOT NULL,
  indicator_region_id INT NOT NULL,
  PRIMARY KEY (indicator_id, indicator_region_id),
CONSTRAINT fk_indicator_data_cualitative_indicator_detail
    FOREIGN KEY (indicator_id , indicator_region_id)
    REFERENCES tb_obs_indicator_detail (detail_indicator_id , detail_region_id)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
-- -----------------------------------------------------
-- Table tb_obs_measurement_unit
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS tb_obs_measurement_unit (
  measurement_unit_id INT AUTO_INCREMENT NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS tb_obs_year (
  ...
  CONSTRAINT fk_year_indicator_data_cuantitative
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_year_measurement_unit
    FOREIGN KEY (measurement_unit_id)
    REFERENCES tb_obs_measurement_unit (measurement_unit_id)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
CREATE TABLE IF NOT EXISTS tb_obs_list (
  ...
  CONSTRAINT fk_list_indicator_data_cualitative
    FOREIGN KEY (indicator_id , indicator_region_id)
    REFERENCES tb_obs_indicator_data_cualitative (indicator_id , indicator_region_id)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_value_measurement_unit
    FOREIGN KEY (measurement_unit_id)
    REFERENCES tb_obs_measurement_unit (measurement_unit_id)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS tb_obs_indicator_reference (
  indicator_id INT NOT NULL,
  indicator_region_id INT NOT NULL,
  indicator_reference_id INT NOT NULL,
  PRIMARY KEY (indicator_id, indicator_region_id, indicator_reference_id),
  CONSTRAINT fk_indicador_detail_has_reference_indicator
    FOREIGN KEY (indicator_id , indicator_region_id)
    REFERENCES tb_obs_indicator_detail (detail_indicator_id , detail_region_id)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_indicator_detail_has_reference_reference
